...
/* delete the current river */
setTimeout(function(){river.deleteRiver("category_river", river.getRiverSettings('category_river'), elasticSearchClient, logger);
    /* wait for deleteRiver - hard coded 30 seconds*/
    /* delete old data/mapping after */
    setTimeout(function(){
        console.log("Finished waiting for delete river; deleting mapping/data");
        /*delete old data and mapping */
        migration.core.deleteMapping(index, 'categoryNode', elasticSearchClient, logger);
        /* wait for delete mapping - hard coded 30 seconds */
        setTimeout(function(){
            /* create mapping */
            console.log("Finished waiting for delete mapping/data; recreating mapping");
            migration.core.putMapping(index, 'categoryNode' ,migration.mapping.categoryNode, elasticSearchClient, logger);
            /* wait for create mapping - hard coded 5 seconds */
            setTimeout(function(){
                console.log("Finished waiting for create mapping; recreating river");
                /* create river */
                river.createRiver("category_river", river.getRiverSettings('category_river'), elasticSearchClient, logger);
                /* create river callback */
